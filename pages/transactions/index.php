<?php
			...
			for ($i=0; $i<$amortizeFor; $i++) {
				...
				do {
					...
					if (isset($_POST[$accountSelectName])) {
						...
						$fullDebit = floatval($fullDebit);
						$debit = $fullDebit / $amortizeFor;
						
						$fullCredit = $cdb->real_escape_string($_POST["credit-".$counter]);
						$fullCredit = floatval($fullCredit);
						$credit = $fullCredit / $amortizeFor;

						$insertSQL = sprintf("INSERT INTO %s (account, debit, credit, date, transaction, entry_num) VALUES ('%s', '%s', '%s', '%s', '%s', '%s')", 
											 $_SESSION["entriesTable"], $account, $debit, $credit, $date, $last_id, $counter);
						$result = runSQL($insertSQL, $cdb);
						if ($result == true) {
		...
		if ($_POST["submit"] == "Distribute Subscription Revenue") {
			...
			do {
				...
				if (isset($_POST[$accountSelectName])) {
					...
					$insertSQL1 = sprintf("INSERT INTO %s (account, credit, date, transaction, entry_num) VALUES ('%s', '%s', '%s', '%s', '%s')", $_SESSION["entriesTable"], $account, $credit, $date, $last_id, $entryCounter);
					...
					$insertSQL2 = sprintf("INSERT INTO %s (account, debit, date, transaction, entry_num) VALUES ('49', '%s', '%s', '%s', '%s')", $_SESSION["entriesTable"], $debit, $date, $last_id, $entryCounter);
			...
			do {
				...
				do {
					...
					if (isset($_POST[$accountSelectName])) {
						...
						$insertSQL = sprintf("INSERT INTO %s (account, debit, credit, date, transaction, entry_num) VALUES ('%s', '%s', '%s', '%s', '%s', '%s')", $_SESSION["entriesTable"], $account, $debit, $credit, $date, $last_id, $counter);
						$result = runSQL($insertSQL, $cdb);
						if ($result == true) {
							$msg .= "Entry for ". $account ." created successfully.";
						} else {
				...
				foreach ($royalties as $royalty) {
					...
					// Credit the artist's account
					$insertSQL = sprintf("INSERT INTO %s (account, debit, credit, date, transaction, entry_num) VALUES ('%s', '%s', '%s', '%s', '%s', '%s')", $_SESSION["entriesTable"], $royalty["credit_account"], "0.00", $amount, $date, $last_id, $counter);
					$result = runSQL($insertSQL, $cdb);
					if ($result == true) {
						$msg .= "Entry for ". $account ." created successfully.";
					} else {
					...
					$insertSQL = sprintf("INSERT INTO %s (account, debit, credit, date, transaction, entry_num) VALUES ('%s', '%s', '%s', '%s', '%s', '%s')", $_SESSION["entriesTable"], $royalty["debit_account"], $amount, "0.00", $date, $last_id, $counter);
					$result = runSQL($insertSQL, $cdb);
					if ($result == true) {
						$msg .= "Entry for ". $account ." created successfully.";
					} else {
		...
		if ($_POST["submit"] == "Import Sales Report") {
			...
			while (($item = fgetcsv($handle, 1000, ",")) !== FALSE) {					
				...
				if ($artists) {
					...
					/* THIS IS NOT CORRECT AND IS NOT NEEDED. ONLY ASSIGN ROYALTIES BELOW.  
					Also, Credit entry was being skipped for some reason. */
					
					/*$debitAccount = $_POST["account-1"];
					$creditAccount = $_POST["account-2"];

					$insertDebitSQL = sprintf("INSERT INTO %s (account, debit, credit, date, transaction, entry_num) VALUES ('%s', '%s', '0.00', '%s', '%s', '%s')", $_SESSION["entriesTable"], $debitAccount, $saleAmount, $date, $last_id, $counter);
		...
		if ($_POST["submit"] == "Edit Transaction") {
			...
			do {
				...
				if (isset($_POST[$accountSelectName])) {
					...
					$testSQL = sprintf("SELECT id FROM %s WHERE transaction = '%s' AND entry_num = '%s'", $_SESSION["entriesTable"], $transactionId, $counter);
					$testEntries = getDataFromTable($testSQL, $cdb);
					if (count($testEntries) > 0) {
					
						// Update existing entry.
						$updateSQL = sprintf("UPDATE %s SET account='%s', debit='%s', credit='%s', date='%s' WHERE transaction = '%s' AND entry_num = '%s'", $_SESSION["entriesTable"], $account, $debit, $credit, $date, $transactionId, $counter);
						$result = runSQL($updateSQL, $cdb);
						if ($result == true) {
						...
						// Add a new entry
						$insertSQL = sprintf("INSERT INTO %s (account, debit, credit, date, transaction, entry_num) VALUES ('%s', '%s', '%s', '%s', '%s', '%s')", $_SESSION["entriesTable"], $account, $debit, $credit, $date, $transactionId, $counter);
						$result = runSQL($insertSQL, $cdb);
						if ($result == true) {
					...
					if (count($testEntries) > 0) {
						...
						$deleteSQL = sprintf("DELETE FROM %s WHERE transaction = '%s' AND entry_num = '%s'", $_SESSION["entriesTable"], $transactionId, $counter);
	...
	$countString = sprintf(" - Last %s transactions", $selectLimit);
	...
	if (isset($_POST["submit"])) {
		if ($_POST["submit"] != "Add Transaction") {
			$transactionsSQL = sprintf("SELECT * FROM `%s` WHERE date >= '%s' AND date <= '%s' ORDER BY date DESC, id DESC %s LIMIT %s", $_SESSION["transactionsTable"], $startDate, $endDate, $desc, $selectLimit);
    ...
    <div id="content-container">
        ...
        <div id="content-area">
			...
			<h3 class="transactions-header">
				<span><?php echo $monthString . $countString; ?></span>
